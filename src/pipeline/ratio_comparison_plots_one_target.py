...
def plot_target(target):
    print(f"\n=== {target} ===")
    plots_dir = Path("ratio_comparison_plots_one_target")
    ...
    cos_ratio_ref      = cos_all.copy()
    cos_ratio_ref_flux = cos_all_flux.copy()
    ...
    for name, (w_list, f_list) in spectra_data.items():
        ...
        try:
            ...
            overlap_min = max(min(w), min(cos_ratio_ref), min_wl_override)
            overlap_max = min(max(w), max(cos_ratio_ref))
    ...
    if len(valid_spectra) == 0:
        if VERBOSE:
            print(f"[SKIPPED] No overlapping instrument data found for {target}")
        return

    num_ratio_plots = len(valid_spectra)
    fig, axs = plt.subplots(
        num_ratio_plots + 1, 1,
        figsize=(22, 6 * (num_ratio_plots + 1)),
        sharex=False,
        gridspec_kw={'height_ratios': [3] + [1] * num_ratio_plots}
    )
    if not isinstance(axs, np.ndarray):
        axs = [axs]

    c, a, z = INSTRUMENT_COLORS["COS"]
    axs[0].plot(bin_centers_cos, binned_cos_disp, marker='o', markersize=8,
    ...
    for name, (w_list, f_list) in valid_spectra.items():
        ...
        axs[0].plot(centers, binned, marker='o', markersize=8,
    ...
    axs[0].set_title(f"{target} {title_suffix}")
    axs[0].legend(ncol = 2)
    axs[0].grid(True)

    for i, (name, (w_list, f_list)) in enumerate(valid_spectra.items(), start=1):
        w = np.concatenate(w_list)
        f = np.concatenate(f_list)
        min_wl_override = 0 if name == "FUSE" else MIN_WAVELENGTH
        ratio_min = max(min(w), min(cos_ratio_ref), min_wl_override)
        ratio_max = min(max(w), max(cos_ratio_ref))

        cos_mask  = (cos_ratio_ref >= ratio_min) & (cos_ratio_ref <= ratio_max)
        inst_mask = (w >= ratio_min) & (w <= ratio_max)
        cos_ov_wl  = cos_ratio_ref[cos_mask]
        cos_ov_fl  = cos_ratio_ref_flux[cos_mask]
        inst_ov_wl = w[inst_mask]
        inst_ov_fl = f[inst_mask]

        if len(cos_ov_wl) == 0 or len(inst_ov_wl) == 0:
            continue

        cos_bins_r  = compute_wavelength_bins(cos_ov_wl,  TARGET_POINTS_PER_BIN * 10)
        inst_bins_r = compute_wavelength_bins(inst_ov_wl, TARGET_POINTS_PER_BIN * 10)

        if len(cos_bins_r) < 2 or len(inst_bins_r) < 2:
            continue

        cos_mean  = avg_bin_size(cos_bins_r)
        inst_mean = avg_bin_size(inst_bins_r)
        ratio_bins   = cos_bins_r  if cos_mean > inst_mean else inst_bins_r
        mean_bin_ang = cos_mean    if cos_mean > inst_mean else inst_mean

        binned_cos_r  = bin_fluxes(cos_ov_wl,  cos_ov_fl,  ratio_bins)
        binned_inst_r = bin_fluxes(inst_ov_wl, inst_ov_fl, ratio_bins)

        if len(binned_cos_r) == 0 or len(binned_inst_r) == 0:
            continue

        ratio   = (10**binned_inst_r - 10**binned_cos_r) / 10**binned_cos_r
        centers = (ratio_bins[:-1] + ratio_bins[1:]) / 2
        median  = np.nanmedian(ratio)
        std     = mad_std(ratio, ignore_nan=True)

        ratio_label = f"({name} - COS) / COS"
        axs[i].plot(centers, ratio, marker='o', markersize=8,
                    color="black", lw=0.8, label=ratio_label)
        axs[i].axhline(median,       color="red",  linestyle="--", label=f"Median: {median:.3f}")
        axs[i].axhline(median + std, color="gray", linestyle=":",  label=f"±1σ: {std:.3f}")
        axs[i].axhline(median - std, color="gray", linestyle=":")
        axs[i].set_xlim(x_min, x_max)
        axs[i].set_ylim(-1, 1)
        axs[i].set_ylabel("Linear Flux Ratio")
